        ...
        if (m.find()){//该行是关键行
            for (int i = 0; i < regexExtract.size(); i++) {
                ...
                if (m.find()){
                    ...
                    //若提供反射处理函数，则处理关键信息
                    if (!UString.isEmpty(regexExtract.get(i).clazz)&&!UString.isEmpty(regexExtract.get(i).method)) {
...
//        regex.add("");
    ...
    private static void testUrlExtract() {
        ...
        beanRegex1.clazz="com.cy.util.UStringFactory";
